...
import { useUpdateExcursionRequestMutation } from '@/globalRedux/features/api/apiSlice';
import { IExcursionRequestResponse } from '@/types/types';
import { toast } from 'react-toastify';

interface UpdateExcursionRequestModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  request: IExcursionRequestResponse | undefined;
  onSave: () => void;
}

const UpdateExcursionRequestModal = ({ open, onOpenChange, request, onSave }: UpdateExcursionRequestModalProps) => {
  ...
  const [excursionDate, setExcursionDate] = useState(
    request?.excursionDate ? new Date(request.excursionDate).toISOString().split('T')[0] : ''
  );
  const [excursionTime, setExcursionTime] = useState(request?.excursionTime || '');
  const [numberOfAdults, setNumberOfAdults] = useState(request?.numberOfAdults?.toString() || '');
  const [numberOfChildren, setNumberOfChildren] = useState(request?.numberOfChildren?.toString() || '');
  const [numberOfBabies, setNumberOfBabies] = useState(request?.numberOfBabies?.toString() || '');
  const [message, setMessage] = useState('');
  const [excursionId, setExcursionId] = useState(request?.excursionId?._id || '');
  const [status, setStatus] = useState<'pending' | 'confirmed' | 'completed' | 'rejected'>(request?.status || 'pending');
  const [paymentPercentage, setPaymentPercentage] = useState<'0' | '100'>(request?.paymentPercentage?.toString() as '0' | '100' || '0');
  const [price, setPrice] = useState(request?.price || 0);
  const [withGuide, setWithGuide] = useState(request?.withGuide || false);
  const [driverLanguages, setDriverLanguages] = useState(request?.driverLanguages || '');
  ...
  useEffect(() => {
    if (request) {
      ...
      setExcursionDate(request.excursionDate ? new Date(request.excursionDate).toISOString().split('T')[0] : '');
      setExcursionTime(request.excursionTime);
      setNumberOfAdults(request.numberOfAdults?.toString() || '');
      setNumberOfChildren(request.numberOfChildren?.toString() || '');
      setNumberOfBabies(request.numberOfBabies?.toString() || '');
      setMessage(request.message || (request.withGuide && request.driverLanguages ? `Langues du Guide: ${request.driverLanguages}` : ''));
      setExcursionId(request.excursionId?._id || '');
      setStatus(request.status);
      setPaymentPercentage(request.paymentPercentage.toString() as '0' | '100');
      setPrice(request.price || 0);
      setWithGuide(request.withGuide || false);
      setDriverLanguages(request.driverLanguages || '');
      setErrors({
        clientName: '',
        clientEmail: '',
        clientPhone: '',
  ...
  }, [excursionId, numberOfAdults, numberOfChildren, numberOfBabies, withGuide, excursions, request]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to update excursion request:', err);
      const errorMessage = err?.data?.message || "Échec de la mise à jour de la demande d'excursion";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg bg-admin-card border-admin-border">
        <DialogHeader>
          <DialogTitle className="text-admin-foreground">Modifier la Demande d'Excursion</DialogTitle>
        </DialogHeader>
        <motion.form
...
export default UpdateExcursionRequestModal;