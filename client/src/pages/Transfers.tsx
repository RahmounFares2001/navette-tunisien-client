...
const formatDateForInput = (): string => {
  return new Date().toISOString().split('T')[0]; // Returns YYYY-MM-DD for today (2025-09-03)
};

const Transfers = () => {
  const { t } = useTranslation();
  const { toast } = useToast();
  const [step, setStep] = useState(1); // Step 1: Locations, Step 2: Vehicle, Step 3: Details
  const [selectedVehicle, setSelectedVehicle] = useState<string>('');
  const [isSuccess, setIsSuccess] = useState(false); // Added to track success state
  ...
  // Calculate max allowed values for numberOfAdults and numberOfChildren
  ...
  const handleSubmit = async () => {
    ...
    // Validate passenger and suitcase limits
    ...
    try {
      ...
      setIsSuccess(true); // Set success state to show confirmation screen
      toast({
